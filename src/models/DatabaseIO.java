...
public class DatabaseIO {
  ...
  static void _writeDatabaseToFiles(Database db) {
    ...
    PrintWriter dbWriter =
        IO.getPrintWriterOrDie(Constants.QUESTION_DATABASE_MAIN_FILE);

    try {
      DatabaseIO._writeDatabaseFile(db, dbWriter);
      Iterator<Question> iter = db.questions.values().iterator();
      while (iter.hasNext()) {
        _writeQuestion(iter.next());
      }
    } catch(IOException ex) {
  ...
  private static void _writeDatabaseFile(Database db, PrintWriter dbWriter)
      throws IOException {
